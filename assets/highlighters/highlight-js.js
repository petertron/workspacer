        ...
        for (var _i = 0; _i < array_in.length; _i++) {
            ...
            if (line_in) {
                ...
                do {
                    ...
                    if(match.index > 0) {
                        frag.appendChild(d.createTextNode(line_in.slice(0, match.index)));
                    ...
                    if (delimiter == "//") {
                        ...
                        frag.appendChild(textSpan(COMMENT, line_in.slice(0, l)));
                        ...
                        frag.appendChild(textSpan(STRING, delimiter));
                        ...
                        if (line_in.length > 0) {
                            ...
                            while (!finished) {
                                ...
                                if (match.index > 0) {
                                    frag.appendChild(textSpan(STRING, line_in.slice(0, match.index)));
                                    line_in = line_in.slice(match.index);
                                }
                                if (match[0] == delimiter) {
                                    ...
                                    frag.appendChild(textSpan(ESCAPE, match[0]));
                    ...
                    /*else if(delimiter == "/") {
                        frag.appendChild(textSpan('regexp_delimiter', delimiter);
                        line_in = line_in.slice(1);
                        if(source_text.length > 0) {
                            var finished = false;
                            while(!finished) {
                                match = /\\\S|\/[gimy]{0,4}|$/.exec(line_in);
                                if(match.index > 0) frag.appendChild(textSpan('', line_in.slice(0, match.index));
                                var m = match[0];
                                if(m.charAt(0) == "/") {
                                    frag.appendChild(textSpan('regexp_delimiter', m);
                                    finished = true;
                                }
                                else if(m.charAt(0) == "\\") {
                        ...
                        frag.appendChild(textSpan((char2 >= "0" && char2 <= "9") ? NUMBER : OBJECT_PART, delimiter));
                        line_in = line_in.slice(delimiter.length);
                    }
                    else if (char1 >= "0" && char1 <= "9") {
                        frag.appendChild(textSpan(NUMBER, delimiter));
                        line_in = line_in.slice(delimiter.length);
                    }
                    else if (delimiter !== "") {
                        var s = (delimiter.search(/[^a-z]/) == -1);
                        if(s && keywords1.indexOf(delimiter) > -1) {
                            frag.appendChild(textSpan(KEYWORD1, delimiter));
                        }
                        else if(s && keywords2.indexOf(delimiter) > -1) {
                            frag.appendChild(textSpan(KEYWORD2, delimiter));
                        }
                        else {
                            frag.appendChild(textSpan('JS_text', delimiter));
