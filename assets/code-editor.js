...
var CodeEditor = $hx_exports["CodeEditor"] = function(settings) {
	...
	this.edit_area.contentEditable = "true";
	this.menu.addEventListener("mousedown",$bind(this,this.menu_onmousedown));
	this.menu.addEventListener("keydown",$bind(this,this.menu_onkeydown));
	...
	this.editor.oncontextmenu = function(event2) {
		...
		if(_gthis.menu.style.visibility == "visible") {
			if(event2.button == 2) {
				_gthis.menu.style.visibility = "hidden";
			}
		} else {
			var menu_top = 0;
			var menu_left = 0;
			if(event2.buttons != 0) {
				if(event2.clientY + _gthis.menu.clientHeight > window.innerHeight) {
					menu_top = event2.clientY - _gthis.menu.clientHeight - 2;
				} else {
					menu_top = event2.clientY + 2;
				}
				if(event2.clientX + _gthis.menu.clientWidth > window.innerWidth) {
					menu_left = event2.clientX - _gthis.menu.clientWidth - 2;
				} else {
					menu_left = event2.clientX + 2;
				}
			}
			_gthis.menu.style.top = menu_top + "px";
			_gthis.menu.style.left = menu_left + "px";
			_gthis.menu.style.visibility = "visible";
		}
	};
};
...
CodeEditor.prototype = {
	...
	,edit_area_onkeydown: function(event) {
		...
		if(event.metaKey || event.ctrlKey) {
			if(event.key == "s") {
				event.preventDefault();
				event.stopPropagation();
				this.editor.dispatchEvent(new CustomEvent("save",{ detail : { text : this.edit_area.textContent}}));
			} else if(event.key == "y") {
				event.preventDefault();
				this.redo();
			} else if(event.key == "z") {
	...
	,renderText: function(text) {
		if(text == null) {
			text = this.edit_area.textContent;
		}
		var line_num_string = "1\n";
		if(text == null) {
			var node = window.document.createTextNode("");
			this.edit_area.innerHTML = "";
			this.edit_area.appendChild(node);
		} else {
			var line_num = 2;
			var index = null;
			var offset = 0;
		...
		this.edit_area.appendChild(window.document.createTextNode(""));
	}
